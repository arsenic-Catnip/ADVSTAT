...
priorcomm = open('comwordslist.txt', 'w')
...
com_words01 = 0			#total number of words that exists both under spam and safe words
			...
			if "spmsg" in filename:
				...
				for line in f:
					...
					for word in f.read().split():
						...
						elif word in spam_words and word not in common_words:
							ham_wordcount[word] += 1
				...
				for line in f:
					...
					for word in f.read().split():
						...
						elif word in safe_words and word not in common_words:
							safe_wordcount[word] += 1
						elif word in common_words:
							comword_wordcount[word] += 1
							com_words01 += 1
...
for word, count in comword_wordcount.items():
	prob = count / len(common_words)
	result = '{:1.10f}'.format(prob)
	formation = "{0} " " {1} {2}".format(str(word), str(count), str(result))
	priorcomm.write(formation + '\n')
	comword_probability[word] = prob

print("prior word probabilities created!\n")
priorspam.close()
priorsafe.close()
priorcomm.close()

#pre-bayes classifier computations
spamclass_prob = spm_mail01 / (spm_mail01 + saf_mail01 + com_words01)	#P(spam)
safeclass_prob = saf_mail01 / (spm_mail01 + saf_mail01 + com_words01)	#P(safe)
